...
options(warn = -1)
library(knitr)
include_graphics("../pics/chosun_field_history.png", dpi = NA)
```
...
prop_field <- colMeans(field, na.rm = TRUE)/sum(colMeans(field, na.rm = TRUE)) * 100
```
...
```{r, reshape}
library(reshape2)
field_melt <- melt(field, 
                   varnames = c("Year", "Province"), 
                   value.name = "Area")
str(field_melt)
# kable(field_melt)
```
