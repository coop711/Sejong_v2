...
library(gridExtra)
# library(extrafont)
load("sejong_poll_data_kr_win_v2.rda")
...
```{r, pie for class2, fig.width = 10, fig.height =5}
par(mfrow = c(1, 2))
# par(mfrow = c(1, 2), family = "Malgun Gothic")
...
```{r, seoul by class, fig.width = 6, fig.height = 5}
# par(family = "Malgun Gothic")
...
```{r, seoul geom_bar stack, fig.width = 8, fig.height = 5}
...
```{r, seoul geom_bar dodge, fig.width = 8, fig.height = 5}
...
```{r, seoul geom_bar fill, fig.width = 8, fig.height = 5}
...
```{r, barplot.gg, fig.width = 8, fig.height = 5}
barplot.gg.stack <- function(df, font.family = "", ggtitle = "", xlab = ""){
x <- df[, 2]
...
barplot.gg.dodge <- function(df, font.family = "", ggtitle = "", xlab = ""){
x <- df[, 2]
y <- unlist(tapply(df$Freq, x, cumsum))
b1 <- ggplot(df, aes(x = x, y = Freq, fill = vote)) +
  geom_bar(stat = "identity", position = "dodge")
...
barplot.gg.fill <- function(df, font.family = "", ggtitle = "", xlab = ""){
x <- df[, 2]
...
barplot.gg <- function(x, position, font.family = "", ggtitle = "", xlab = ""){
  switch(position,
...
```{r, mosaicplot for seoul, fig.width = 8, fig.height = 5}
# par(family = "Malgun Gothic")
...
```{r, barplot for regional bureaus, fig.width = 8, fig.height = 5}
# par(family = "Malgun Gothic")
...
```{r, geom_bar for regional bureaus, fig.width = 8, fig.height = 5}
vote.region.bureaus.df <- as.data.frame(vote.region.bureaus[, -1])
barplot.gg(vote.region.bureaus.df, position = "stack", ggtitle = "지방 관료들의 찬반", xlab = "지역")
```

### `mosaicplot()`의 활용

```{r, mosaic for local bureaus, fig.width = 8, fig.height = 5}
# par(family = "Malgun Gothic")
...
```{r, barplot for commons, fig.width = 8, fig.height = 5}
# par(family = "Malgun Gothic")
...
```{r, geom_bar for commons, fig.width = 12, fig.height = 5}
vote.region.commons.df <- as.data.frame(vote.region.commons)
barplot.gg(vote.region.commons.df, position = "stack", ggtitle = "품관촌민의 찬반", xlab = "지역")
```

### `mosaicplot()`의 활용

```{r, mosaicplot for commons, fig.width = 8, fig.height = 5}
# par(family = "Malgun Gothic")
...
```{r, barplot for chungcheong, fig.width = 8, fig.height = 5}
...
legend("right", inset = -0.2, fill = rainbow.2, legend = c("찬성", "반대"))
text(x = c(0.6, 1.85, 1.85, 3.1, 3.1), y = c(0.5, 0.3, 0.8, 0.15, 0.65), labels = format(c(2, 35, 26, 6982, 14013), big.mark = ","))
```

#### ggplot

```{r, geom_bar fill for Chungcheong, fig.width = 8, fig.height = 5}
vote.chung.class.df <- as.data.frame(vote.chung.class)
barplot.gg(vote.chung.class.df, position = "fill", ggtitle = "충청도의 찬반", xlab = "계급")
```
...
```{r, mosaic chungcheong, fig.width = 8, fig.height = 5}
# par(family = "Malgun Gothic")
