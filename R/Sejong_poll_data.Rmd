...
par(mfrow=c(1,2))
pie(xtabs(counts~vote+class.2, data=sejong.poll.2[class.2=="Bureaus",], drop=T), labels=c("yes", "no"))
title(main="Bureacrats by vote")
text(x=0, y=c(0.4,-0.4), labels=c("915", "845"))
pie(xtabs(counts~vote+class.2, data=sejong.poll.2[class.2=="Commons",], drop=T), labels=c("yes", "no"))
...
```{r}
barplot(xtabs(counts~vote+class, data=sejong.poll.2[region=="Seoul",], drop=T))
title(main="Seoul by vote")
text(x=c(0.7, 1.9, 1.9, 3.1, 3.1), y=c(120, 450, 135, 500, 220), labels=c("194","393", "259", "117", "443"))
legend("topleft", inset=0.05, legend=c("yes", "no"))
```

- Using mosaicplot()

```{r}
mosaicplot(xtabs(counts~class+vote, data=sejong.poll.2[region=="Seoul",], drop=T), main="Seoul by vote")
```
...
barplot(xtabs(counts~vote+region, data=sejong.poll.2[class.2=="Bureaus" & !region=="Seoul",], drop=T))
...
```{r fig.width=12, fig.height=6}
mosaicplot(xtabs(counts~region+vote, data=sejong.poll.2[class.2=="Bureaus" & !region=="Seoul",], drop=T), main="")
title(main="Bureacrats' vote by region other than Seoul")
```
...
xtabs(counts~vote+region, data=sejong.poll.2[class.2=="Commons",], drop=T)
barplot(xtabs(counts~vote+region, data=sejong.poll.2[class.2=="Commons",], drop=T))
...
```{r fig.width=12, fig.height=6}
mosaicplot(xtabs(counts~region+vote, data=sejong.poll.2[class.2=="Commons",], drop=T), main="Commons' votes by region")
```
...
barplot(prop.table(xtabs(counts~vote+class, data=sejong.poll.2[region=="Chungcheong",], drop=T), margin=2), ylim=c(0, 1.5), axes=F)
...
```{r}
mosaicplot(xtabs(counts~class+vote, data=sejong.poll.2[region=="Chungcheong",], drop=T), main="")
