...
```{r, array structure}
sejong.poll.array <- xtabs(counts ~ vote + class + region, 
                           data = sejong.poll.2)
...
```{r, total yes or no}
vote.total <- xtabs(counts ~ vote, 
                    data = sejong.poll.2)
kable(t(as.matrix(vote.total)), 
      caption = "Total")
# format(prop.table(vote.total)*100, digits = 3, nsmall = 1)
kable(t(as.matrix(format(prop.table(vote.total) * 100, 
                         digits = 3, 
                         nsmall = 1))), 
      caption = "Percentage", 
      align = rep("r", 2))
...
```{r, by class}
vote.class <- xtabs(counts ~ vote + class, 
                    data = sejong.poll.2)
kable(vote.class, 
      caption = "By Class")
...
sejong.poll.2$class.2 <- factor(ifelse(sejong.poll.2$class == "Commons", 
                                       "Commons", "Bureaus"), 
...
```{r, vote by class.2}
vote.class.2 <- xtabs(counts ~ vote + class.2, 
                      data = sejong.poll.2)
...
vote.region.bureaus <- xtabs(counts ~ vote + region, 
                             data = sejong.poll.2, 
                             class.2 == "Bureaus", 
                             drop = TRUE)
