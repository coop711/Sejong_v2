...
tbl.class <- xtabs(소계 ~ 계급, data = sejong.poll.kr)
tbl.region <- xtabs(소계 ~ 지역, data = sejong.poll.kr)
tbl.vote.class <- xtabs(소계 ~ 찬반 + 계급, data = sejong.poll.kr)
tbl.vote.region <- xtabs(소계 ~ 찬반 + 지역, data = sejong.poll.kr)
tbl.vote.class.region <- xtabs(소계 ~ 찬반 + 계급 + 지역, data = sejong.poll.kr)
ftbl.vote.class.region <- ftable(xtabs(소계 ~ 찬반 + 계급 + 지역, data = sejong.poll.kr))
```
...
```{r, array structure}
sejong.poll.array <- xtabs(소계 ~ 찬반 + 계급 + 지역, data = sejong.poll.kr)
...
```{r, total yes or no}
vote.total <- xtabs(소계 ~ 찬반, data = sejong.poll.kr)
...
```{r, by class}
vote.class <- xtabs(소계 ~ 찬반 + 계급, data = sejong.poll.kr)
...
sejong.poll.kr$class.2 <- factor(ifelse(sejong.poll.kr$"계급" == "품관촌민", "품관촌민", "관료"), levels = c("관료", "품관촌민"))
...
```{r, vote by class.2}
vote.class.2 <- xtabs(소계 ~ 찬반 + class.2, data = sejong.poll.kr)
...
vote.region.bureau <- xtabs(소계 ~ 찬반 + 지역, data = sejong.poll.kr, class.2 == "관료", drop = TRUE)
kable(vote.region.bureau, caption = "집계(관료)")
# xtabs(소계 ~ 찬반 + 지역, data = sejong.poll.kr[class.2 == "Bureaus", ], drop = TRUE)
vote.region.commons <- xtabs(소계 ~ 찬반 + 지역, data = sejong.poll.kr, class.2 == "품관촌민", drop = TRUE)
...
```{r, Seoul}
region <- sejong.poll.kr$"지역"
vote.seoul.class <- xtabs(소계 ~ 찬반 + 계급, data = sejong.poll.kr, region == "서울", drop = TRUE)
...
```{r, Chungcheong}
vote.chung.class <- xtabs(소계 ~ 찬반 + 계급, data = sejong.poll.kr, region == "충청", drop = TRUE)
...
```{r, save}
save.image(file="sejong_poll_data_kr_2.rda")
```
