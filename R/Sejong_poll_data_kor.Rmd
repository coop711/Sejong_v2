...
sejong.poll.kr$class.2 <- factor(ifelse(sejong.poll.kr$class == "품관촌민", "평민", "관료"), levels = c("관료", "평민"))
...
kable(vote.class.2, caption = "관료와 평민")
vote.class.2.a <- cbind("관료" = rowSums(vote.class.a[, -5]), "평민" =  vote.class.a[, 5])
kable(vote.class.2.a, caption = "관료와 평민")
```
...
```{r, proportions}
kable(format(prop.table(vote.class.2, margin = 2)*100, digits = 3, nsmall = 1), caption = "관료와 평민", align = rep("r", 2))
```
...
vote.region.bureau <- xtabs(counts ~ vote + region, data = sejong.poll.kr, class.2 == "관료", drop = TRUE)
kable(vote.region.bureau, caption = "집계(관료)")
# xtabs(counts ~ vote + region, data = sejong.poll.kr[class.2 == "Bureaus", ], drop = TRUE)
vote.region.commons <- xtabs(counts ~ vote + region, data = sejong.poll.kr, class.2 == "평민", drop = TRUE)
kable(vote.region.commons, caption = "집계(평민)", align = rep("r", 9))
```
